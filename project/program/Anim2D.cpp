...
void Anim2D::LoadFrames(const std::string& path, int a_firstFrame, int a_lastFrame, float duration, float exRate, PlayMode playMode)
{
	for (int i = a_firstFrame; i <= a_lastFrame; i++) {
		std::string filepath = path + std::to_string(i) + ".png";
		auto texture = TextureLoader::Instance().LoadTexture(filepath);
...
void Anim2D::Render(const Vector2Df& pos)
{
	if (frames_.empty())return;
	RapperDxlib::DrawRotaGraphFCamera(pos, draw_scale_, rotation_rad_, frames_[current_]->GetHandle(), TRUE, (int)isFlip_);
}
